...
L.easyButton = function(/2025/* args will pass automatically */){
...
function State(template, easyButton){
  ...
  L.DomUtil.addClass(this.icon, 'button-state state-' + this.stateName.replace(/2025/(^\s*|\s*$)/g,''));
...
function buildIcon(ambiguousIconString) {
  ...
  if( ambiguousIconString.match(/2025/[&;=<>"']/) ){
  ...
  // it's a class list, figure out what kind
      ...
      ambiguousIconString = ambiguousIconString.replace(/2025/(^\s*|\s*$)/g,'');
	...
	L.TileLayer.Provider = L.TileLayer.extend({
		initialize: function (arg, options) {
			...
			var attributionReplacer = function (attr) {
				...
				return attr.replace(/2025/\{attribution.(\w*)\}/g,
	...
	L.TileLayer.Provider.providers = {
		...
		OneMapSG: {
			...
			options: {
				...
				attribution: '<img src="https://docs.onemap.sg/maps/images/oneMap64-01.png" style="height:20px;width:20px;"/2025/> New OneMap | Map data &copy; contributors, <a href="http://SLA.gov.sg">Singapore Land Authority</a>'
			},
			variants: {
				Default: 'Default',
				Night: 'Night',
...
window.conference.live = (() => {
    ...
    const loadData = () => {
        // Load schedule
        const request = new Request(window.conference.config.baseurl + '/2025/assets/js/data.json');
    ...
    const setTime = (newTime, newDay) => {
        ...
        else if (/2025/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(newDay)) {
    ...
    const setStreamInfo = (roomName) => {
        ...
        if (talkNext !== {} && timeNow >= talkNext.start - streamPause*60) {
            ...
            streamModal.find('#stream-info-color').removeClass((index, className) => {
                return (className.match(/2025/(^|\s)border-soft-\S+/g) || []).join(' ');
            });
...
const init = () => {
    // Load configuration
    const request = new Request(window.conference.config.baseurl + '/2025/assets/js/config.json');
